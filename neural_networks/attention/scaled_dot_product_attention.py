...
class ScaledDotProductAttention:
    ...
    This is a core component of Transformer models, allowing the model to weigh
    the importance of different parts of the input sequence when processing
    information. The attention score is calculated as:
    Attention(Q, K, V) = softmax( (Q K^T) / sqrt(d_k) ) V
    where Q, K, V are Query, Key, and Value projections respectively, and d_k
    is the dimension of the Key vectors.

    Attributes:
        proj_layer (Dict[str, Projection]): Not directly used by this class if
            Q, K, V are pre-computed. Retained for potential future use or
            context.
        dim_k (Optional[ARRAY_TYPE]): Dimension of the key vectors, stored
            during forward pass.
        projections (Dict[str, ARRAY_TYPE]): Stores Q, K, V projections during
            forward pass.
        softmax (Softmax): Instance of the Softmax activation function.
        softmax_attn (ARRAY_TYPE): Attention weights after applying softmax,
            stored during forward pass.
        attention (ARRAY_TYPE): Output of the attention mechanism, stored
            during forward pass.
    """
    ...
    def __init__(self, softmax_do_clip: bool = False):
        ...
        Args:
            softmax_do_clip (bool, optional): Whether the Softmax function used
                internally should clip its output. Defaults to False, as
                clipping is often not needed before multiplication with Value
                vectors if CrossEntropyLoss handles logits.
        """
        self.dim_k: Optional[ARRAY_TYPE] = None
        self.projections: Dict[str, ARRAY_TYPE] = {}
        # Softmax is applied on attention scores. Clipping is usually not
        # needed here unless the output is directly fed into a log function
        # elsewhere.
    ...
    def forward(
        self, q_proj: ARRAY_TYPE, k_proj: ARRAY_TYPE, v_proj: ARRAY_TYPE
    ) -> ARRAY_TYPE:
        r"""
        ...
        Args:
            q_proj (ARRAY_TYPE): Query projection.
                Shape: (batch_size, seq_len_q, dim_q)
            k_proj (ARRAY_TYPE): Key projection.
                Shape: (batch_size, seq_len_kv, dim_k)
            v_proj (ARRAY_TYPE): Value projection.
                Shape: (batch_size, seq_len_kv, dim_v)
        ...
        Returns:
            ARRAY_TYPE: The attention output.
    ...
    def backprop(
        ...
        Given \( dA = \frac{\partial L}{\partial A} \) (gradient of loss w.r.t.
        attention output), this computes
        \( dQ_{proj}, dK_{proj}, dV_{proj} \)
        (gradients w.r.t. Q, K, V inputs to this block).

        Let \( A = \text{softmax}(S) V \),
        ...
        3.  Gradient w.r.t. scaled scores (\(S\)
            (passing through softmax.backprop):
            Let \( dS' = d\text{softmax}(S) \).
            .. math::
                dS = \text{softmax.backprop}(dS')
                   = \text{softmax}(S) \odot (dS' - \text{sum}
                    (dS' \odot \text{softmax}(S)))
        ...
        Args:
            dA (ARRAY_TYPE): Gradient of the loss w.r.t the attention output.
                             Shape: (batch_size, seq_len_q, dim_v).

        Returns:
            Tuple[ARRAY_TYPE, ARRAY_TYPE, ARRAY_TYPE]:
                - \( dQ_{proj} \): Gradient w.r.t. Query projection.
                    Shape: (batch_size, seq_len_q, dim_q)
                - \( dK_{proj} \): Gradient w.r.t. Key projection.
                    Shape: (batch_size, seq_len_kv, dim_k)
                - \( dV_{proj} \): Gradient w.r.t. Value projection.
                    Shape: (batch_size, seq_len_kv, dim_v)
        """
        if (
            self.softmax_attn is None
            or self.projections.get("value") is None
            or self.projections.get("key") is None
        ...
        # dScaledScores = self.softmax.backprop(dSoftmaxAttn)
        # Need to reshape for softmax backprop if it expects 2D input
        ...
        # self.softmax internal state (_activation) was set with reshaped input
        # during forward. So, dSoftmaxAttn_reshaped is appropriate here.
