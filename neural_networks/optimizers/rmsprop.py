...
class RMSProp(Optimizer):
    ...
    def __init__(
        ...
        Args:
            ...
            rho (float, optional): The discounting factor for the moving
                average of squared gradients (\(\rho\)). Defaults to 0.9.
            epsilon (float, optional): A small constant (\(\epsilon\)) added
                to the denominator for numerical stability. Defaults to 1e-7.
        """
        super().__init__()
        ...
        if not 0.0 <= rho < 1.0:
            raise ValueError(
                f"Invalid rho (discounting factor): {rho}, must be in [0, 1)."
            )
        if not 0.0 < epsilon:
    ...
    def _initialize_history(
        ...
        Returns:
            Dict[str, ARRAY_TYPE]: Dictionary with "accum_sq_grad" initialized
                to zeros.
        """
        backend, _ = get_backend()
        # 'accum_sq_grad' stores E[g^2]_t, the moving average of squared
        # gradients.
    ...
    def optimize(
        ...
        Args:
            history (Optional[Dict[str, ARRAY_TYPE]]): Optimizer history,
                expects "accum_sq_grad". Initialized if None.
