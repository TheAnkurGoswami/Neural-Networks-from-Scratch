...
class Adam(Optimizer):
    ...
    def __init__(
        ...
        Args:
            learning_rate (float, optional): The learning rate (\(\eta\)). Defaults to 0.001.
            beta_1 (float, optional): The exponential decay rate for the first moment estimates (mean of gradients).
                                     Defaults to 0.9.
            beta_2 (float, optional): The exponential decay rate for the second moment estimates (variance of gradients).
                                     Defaults to 0.999.
            epsilon (float, optional): A small constant (\(\epsilon\)) for numerical stability, added to the
                                       denominator. Defaults to 1e-8.
        """
        super().__init__()
        ...
        # _t is the timestep, used for bias correction. Adam is stateful per parameter update.
        # Note: self._epoch is set by set_cur_epoch and typically refers to passes over the dataset.
        # Adam's t is more like an iteration counter.
        self._t: int = 0
    ...
    def optimize(
        ...
        \( m_t \) be the first moment vector (mean), and \( v_t \) be the second moment vector (uncentered variance).
        ...
        Args:
            history (Optional[Dict[str, ARRAY_TYPE]]): Optimizer history, expects "first_moment_t" and "second_moment_t".
                                                     Initialized if None.
        ...
        Returns:
            Tuple[ARRAY_TYPE, Dict[str, ARRAY_TYPE]]:
                ...
                - The updated history dictionary with new "first_moment_t" and "second_moment_t".
        """
        self._t += 1  # Increment timestep for bias correction
        ...
        second_moment_t = (
            ...
            + (1 - self._beta2) * backend.square(derivative) # Using backend.square for clarity
        )

        # Compute bias-corrected first moment estimate
        # m_hat_t = m_t / (1 - beta1^t)
        first_mom_corr_factor = 1 - self._beta1**self._t
        corrected_first_moment_t = first_moment_t / first_mom_corr_factor

        # Compute bias-corrected second raw moment estimate
        # v_hat_t = v_t / (1 - beta2^t)
        second_mom_corr_factor = 1 - self._beta2**self._t
        corrected_second_moment_t = second_moment_t / second_mom_corr_factor
