...
from neural_networks.losses.mse_loss import MSELoss
...
class RMSELoss(MSELoss):
    def __init__(self) -> None:
        ...
        Attributes:
            _loss (Optional[ARRAY_TYPE]): The RMSE loss value computed in the forward pass.
                                          Inherits _y_true, _y_pred, _size from MSELoss.
        """
        super().__init__()  # Initialize MSELoss attributes like _y_true, _y_pred, _size
    ...
    def forward(self, y_pred: ARRAY_TYPE, y_true: ARRAY_TYPE) -> NUMERIC_TYPE:
        ...
        where:
         - \(N\) is the total number of elements (inherited from MSELoss as `_size`).
        ...
        mse_loss = super().forward(y_pred, y_true) # This sets _y_pred, _y_true, _size in parent

        # Add a small epsilon for numerical stability if mse_loss can be zero,
        # though sqrt of a small positive number is fine.
        # The primary concern is if mse_loss could be negative due to precision, but mean of squares shouldn't be.
        epsilon = 1e-12
    ...
    def backprop(self) -> ARRAY_TYPE:
        ...
        epsilon = 1e-12 # Should be consistent with forward pass if used there for sqrt

        # If RMSE (self._loss) is effectively zero, the gradient might be undefined or extremely large.
        # The (self._loss + epsilon) handles the division by zero.
        # If self._loss is exactly 0 and mse_backprop is also 0, result is 0.
        # If self._loss is 0 but mse_backprop is not, result is large (capped by epsilon effect).
